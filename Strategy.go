...
type Strategy struct {
	PairMap map[DealerHand]map[PlayerHand]PlayerAction
	SoftMap map[DealerHand]map[PlayerHand]PlayerAction
	HardMap map[DealerHand]map[PlayerHand]PlayerAction
}
...
func (strategy *Strategy) Play(playerHand Hand, dealerHand Hand) PlayerAction {
	...
	if playerHand.IsPair() {
		return strategy.PairMap[DealerHand(dealerScore.Hard)][PlayerHand(playerScore.Hard)]
	}

	if playerHand.HasSoftValue() {
		return strategy.SoftMap[DealerHand(dealerScore.Hard)][PlayerHand(playerScore.Soft)]
	}

	return strategy.HardMap[DealerHand(dealerScore.Hard)][PlayerHand(playerScore.Hard)]
}
...
func stringToMap(raw []byte, columns []DealerHand, rows []PlayerHand) map[DealerHand]map[PlayerHand]PlayerAction {
	parsedMap := make(map[DealerHand]map[PlayerHand]PlayerAction)

	cursor := 0
	for _, column := range columns {
		parsedMap[column] = make(map[PlayerHand]PlayerAction)

		for _, row := range rows {
			parsedMap[column][row] = PlayerAction(raw[cursor])
			cursor += 1
		}
	}
